...
public class AADClaimsVerifier implements JwtClaimsSetVerifier {
    ...
    private static final String AUD_ROLES = "roles";
    ...
    public AADClaimsVerifier(final String[] aadAliases, final String[] acceptedTenants, final String applicationId, final String[] appRoles) {
        ...
        Assert.notEmpty(appRoles, "appRoles cannot be empty");
        for (final String tenant : appRoles) {
            Assert.notNull(tenant, "appRoles value cannot be null");
        }
    ...
    public void verify(final Map<String, Object> claims) throws InvalidTokenException {
        ...
        if (!claims.containsKey("roles"))
            throw new InvalidTokenException("token must contain (roles) claim");
        ...
        final ArrayList<String> jwtRoles = (ArrayList<String>) claims.get(AUD_ROLES);
        if (!Arrays.asList(acceptedRoles).containsAll(jwtRoles)) {
            throw new InvalidTokenException("Invalid Roles (roles) claim: " + jwtRoles);
        }
